...
# Try to import QuantLib, but don't fail if not available
try:
...
class PortfolioManagementEngine:
    ...
    def __init__(self, 
                 ...
                 performance_window: int = 252):  # Trading days in a year
        ...
        self.metrics_update_interval: float = 3600  # 1 hour
        ...
        try:
            ...
            if symbol in self.positions:
                ...
                if (position.quantity > 0 and quantity < 0) or (position.quantity < 0 and quantity > 0):
                    # Opposite direction trade - calculate realized P&L
                ...
                if abs(position.quantity + quantity) < 1e-8:
                    ...
                    # Update weighted average entry price if adding to position
    ...
    async def _calculate_performance_metrics(self):
        ...
        try:
            ...
            current_time = time.time()
            period_start = current_time - (len(self.daily_returns) * 24 * 3600)  # Approximate
            ...
            # Use empyrical for calculations, with fallbacks
            try:
            ...
            # Market comparison metrics (if benchmark available)
        ...
        try:
            ...
            for symbol, target_weight in target_weights.items():
                ...
                if symbol in self.positions:
                    ...
                    current_value = 0
                    current_price = 50000  # Default price - should get from market data
                
                value_difference = target_value - current_value
                quantity_difference = value_difference / current_price
                
                if abs(quantity_difference) > 0.001:  # Minimum rebalance threshold
            ...
            # Return rebalance orders (execution would be handled by order execution engine)
