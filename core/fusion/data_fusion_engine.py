...
class FusionSignal:
    symbol: str
    signal_type: str  # momentum_sentiment, breakout_buzz, etc.
...
class DataFusionEngine:
    ...
    async def ingest_sentiment_event(self, event: SentimentEvent):
        ...
        # Trigger correlation analysis for relevant symbols
    ...
    async def _analyze_correlations(self, symbol: str):
        ...
        if correlation_result:
            # Generate signals if correlation is significant
    ...
    def _calculate_correlations_fixed(self, price_data: List[PriceEvent], 
        ...
        try:
            ...
            # Create aligned sentiment scores
            sentiment_scores = []
            
            # For each price return, find the closest sentiment
            for i, price_event in enumerate(price_data[1:], 1):
                ...
                if closest_sentiment:
                    ...
                    sentiment_scores.append(0.0)  # Neutral if no close match
    ...
    async def _generate_signals(self, symbol: str, correlation_data: Dict, 
        ...
        # Signal 1: Momentum-Sentiment Alignment
        if abs(correlation) > 0.3:  # Significant correlation
            alignment_strength = min(abs(correlation), 1.0)
            
            # Both momentum and sentiment pointing same direction
        ...
        # Signal 2: Sentiment-Price Divergence (contrarian signal)
