...
class MockFusionSignalGenerator:
    ...
    def __init__(self, alpha_engine: AlphaDetectionEngine):
        ...
        self.running = False
        self.price_history = [45000.0]  # Starting BTC price
    ...
    def _generate_mock_signal(self) -> dict:
        ...
        price_change = random.gauss(0, 0.01)  # 1% std dev
        ...
        # Add some correlation with price momentum
        ...
        if len(self.sentiment_history) >= 10 and len(self.price_history) >= 10:
            ...
            sentiment_recent = self.sentiment_history[-9:]  # Match length
        ...
        # Calculate actual future return for training (this would come from market data)
...
async def main():
    ...
    # Initialize alpha detection engine
    alpha_engine = AlphaDetectionEngine(
        prediction_horizon=15,  # 15 minutes ahead
        feature_window=60,      # 60 minutes of features
        retrain_interval=120    # Retrain every 2 minutes for testing
    )
    ...
    try:
        ...
        # Wait and show periodic summaries
